...
export function GameplayUI({ level, gameState, onSubmit, onFeedbackDone }: GameplayUIProps) {
  const [inputValue, setInputValue] = useState('');
  const [shakeKey, setShakeKey] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);

  const currentQuestion: Question | undefined = gameState.questions[
  ...
  }, [gameState.showFeedback, gameState.currentIndex]);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4">
      ...
      <div className="text-center mb-6">
        ...
        <p className="font-im-fell text-sm opacity-70 mt-1">{LEVEL_SUBTITLES[level]}</p>
      </div>
      ...
      <ZeldaPanel className="p-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex flex-col items-start">
            <span className="font-cinzel text-xs font-bold opacity-60 uppercase tracking-widest mb-1">Lives</span>
            <HeartIcons total={3} filled={gameState.lives} />
          </div>
          <div className="flex flex-col items-end">
            <span className="font-cinzel text-xs font-bold opacity-60 uppercase tracking-widest mb-1">Question</span>
        ...
        <div className="text-center mb-2">
          <span className="font-cinzel text-sm font-bold opacity-60">
            Score: <span className="text-zelda-green font-bold">{gameState.score}</span> / 10
          </span>
        </div>
